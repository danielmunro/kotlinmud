...
class ContextBuilderService(
    ...
    fun createContext(syntax: Syntax, request: RequestService, word: String): Context<out Any> {
        val context = when (syntax) {
            ...
            Syntax.AVAILABLE_NOUN -> AvailableNounContextBuilder(itemService, request.getMob(), request.room).build(syntax, word)
            Syntax.TARGET_MOB -> TargetMobContextBuilder(mobService, request.getMob(), request.room).build(syntax, word)
            Syntax.OPTIONAL_TARGET -> OptionalTargetContextBuilder(request.getMob(), findMobsForRoom(request.room) + itemService.findAllByOwner(request.getMob())).build(syntax, word)
            ...
            Syntax.ITEM_FROM_MERCHANT -> ItemFromMerchantContextBuilder(itemService, request.getMob(), findMobsForRoom(request.room)).build(syntax, word)
            Syntax.ITEM_TO_SELL -> ItemToSellContextBuilder(itemService, request.getMob(), findMobsForRoom(request.room)).build(syntax, word)
            ...
            Syntax.AVAILABLE_DRINK -> AvailableDrinkContextBuilder(itemService, request.getMob(), request.room).build(syntax, word)
            ...
            Syntax.AVAILABLE_ITEM_INVENTORY -> AvailableItemInventoryContextBuilder(request.getMob(), request.room, itemService).build(syntax, word)
